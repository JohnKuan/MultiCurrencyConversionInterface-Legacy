...
extension Reactive where Base: CurrencyTextField {
    ...
    public var rx_text: Binder<String?>{
        return Binder(self.base) { label, text in
            label.text = text
        }
    }
    
    public var rx_value: Binder<Double>{
...
extension CurrencyTextField {
    func changeCurrentLocal(code: String) {
        currencyFormatter.locale = CurrencyConverter.getLocalForCurrencyCode(code: code)
    }
